...
GitterBot.prototype.addParser = function(parser) {
  if(!parser.execute || !parser.help) { // monkey checking
...
GitterBot.prototype.listenRoom = function(roomId) {
  ...
  this._gitter.rooms.join(roomId)
    ...
    .fail(function(err) {
      console.error('Not possible to join the room:', roomId, err);
    });
...
/**
 * Collect and return parsers commands.
 * @returns {string}
 * @private
 */
GitterBot.prototype._collectHelp = function() {
  ...
  var result = [], help;
  result.push('Available commands:');
